		...
		shoping:function(options){
			...
			var S={
				...
				addShoping:function(e){
					...
					var $target=$(e.target),
						...
						if(!($(document).find("#cartIcon").length>0)){
							...
							var $obj=$('#cartIcon');
							if(!$obj.is(':animated')){
								$obj.css({'left': x,'top': y}).animate({'left': X-80,'top': Y-90},500,function() {
				...
				addIcon:function(){
					...
					var icon = '<div id="cartIcon" style="width:110px;height:110px;background:#fff;border:solid 2px #e54144;overflow:hidden;position:absolute;z-index:890;'+$options.iconCSS+'"><img src="'+$options.iconImg+'" width="20" height="20" /></div>';
					return icon;
				}
			};
			S.init(); 
